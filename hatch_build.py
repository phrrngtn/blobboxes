...
separate them: `libbboxes.so` there may be a stale Linux core, while
`bboxes.so` is the SQLite extension and is genuinely correct on macOS
...
# are bboxes.* (the SQL function prefix) while the importable package is

...
const char* bboxes_xls_formulas_json_file(const char* path);
...
bboxes_cursor* bboxes_open_xls(const void* buf, size_t len,
                                const char* password,
                                int start_page, int end_page);
...
#define BBOXES_FORMAT_XLS          8  /* legacy .xls (BIFF/OLE2) via libxls */
